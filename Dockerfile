...
# System deps: chromadb's default (ONNX) embedding function needs onnxruntime,
# which links against libgomp. build-essential covers any wheels that compile.
...
# Copy the app + the prebuilt corpus (rag/chroma_db is ~2 MB).
COPY . .
